...
namespace kpeg
{
    ...
    struct Node
    {
        ...
        Node(const std::string _code, const kpeg::types::UInt16 _val) :
         root{ false } ,
         leaf{ false } ,
         code{ _code } ,
         value{ _val } ,
        ...
        // Only the leaves with a Huffman code value are set to true
        bool leaf;

        // The bitstream binary code (e.g., 1101011...)
        std::string code;

        // The Huffman code (e.g., 0xC3)
        kpeg::types::UInt16 value;
    ...
    inline NodePtr createRootNode(const kpeg::types::UInt16 value)
    {
    ...
    class HuffmanTree
    {
        public:
            ...
            HuffmanTree(const kpeg::types::HuffmanTable& htable);

            /// Create a Huffman tree using specified Huffman table
            ///
            /// @param htable the Huffman table to use
            void constructHuffmanTree(const kpeg::types::HuffmanTable& htable);
