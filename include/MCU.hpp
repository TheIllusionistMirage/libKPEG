    ...
    class MCU
    {
        public:
            ...
            MCU(const std::array<std::vector<int>, 3>& compRLE,
                const std::vector<std::vector<kpeg::types::UInt16>>& QTables);
            ...
            /// Get the pixel array for the Y (luminance) component
            const Matrix8x8 getYMatrix() const;
            
            /// Get the pixel array for the Cb (chroma) component
            const Matrix8x8 getCbMatrix() const;
            
            /// Get the pixel array for the Cr (chroma) component
        ...
        private:
            ...
            /// Conver the MCU's underlying pixels from the Y-Cb-Cr color model to RGB color model
        ...
        private:
            ...
            /// The order of the MCU among all MCUs in the image
            static int m_MCUCount;
            
            /// The quantization table used for de-quantization
            static std::vector<std::vector<kpeg::types::UInt16>> m_QTables;
